...
function patches.prepare() -- GTA:SA pre-game patches
    if memory.getuint32(0xC8D4C0, false) < 9 then
      is_patched = false
      memory.copy(0x866CCC, memory.strptr("slmp_load"..string.char(0x0)), 10) -- drop loadscreen
      ...
      addEventHandler("onD3DPresent", function()
        if memory.getuint32(0xC8D4C0, true) == 7 and not is_patched then -- drop loadscreens
          is_patched = true
