...
function chat_pool:process(text)
  ...
  elseif text:match("^/quit%s*$") or text:match("^/q%s*$") then
    --packets.send(packets["list"]["ID_DISCONNECTION_NOTIFICATION"], nil) // TODO: change this
...
imgui.OnFrame(function() return not isPauseMenuActive() and chat_pool.mode_num > 0 end, -- and not scoreboard.enable //TODO: scoreboard
function()
...
function(self)
  ...
  while clipper:Step() do
    for i = clipper.DisplayStart + 1, clipper.DisplayEnd do
      local color = ("{%x}"):format(chat_pool.messages[i].color)
    ...
    ffi.sizeof(chat_pool.input), imgui.InputTextFlags.EnterReturnsTrue) then
      ...
      if output:sub(1, 1) == "/" then
        if chat_pool:process(output) then
          client:sendCommand(u8:decode(output))
        end
      elseif #output > 0 then
        client:sendChat(u8:decode(output))
      end
