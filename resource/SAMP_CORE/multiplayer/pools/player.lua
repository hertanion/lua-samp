...
function players.remove(index)
    print(index, players.list[index])
...
function players.spawn(index)
    if index - 1 == localplayer:GetLocalPlayerID() then return end
    ...
    if is_special then
      chat_pool:add(0xFF0000, "[Warning] WorldPlayerAdd invalid model! PlayerID: " .. index - 1)
      skin = 0
    end
    ...
    requestModel(skin)
    loadAllModelsNow()
    ptr.ped = createChar(21, skin, 0, 0, 0)
...
function players.nametags()
    ...
    for i, v in ipairs(players.list) do
      if v ~= 0 and doesCharExist(v.ped) and isCharOnScreen(v.ped) and not isPauseMenuActive() then
        ...
        if not result then
          if dist <= 20 then
            ...
            renderFontDrawText(nametag_font, v.nickname .. " (" .. i - 1 .. ")", wposX - renderGetFontDrawTextLength(nametag_font, v.nickname .. " (" .. i - 1 .. ")") / 2, wposY, 0xFFFFFFFF)
